(function(){
  var self, pageMod, notifications, tabs, Request, indexedDB, ActionButton, Panel, ref$, setTimeout, clearTimeout, prefs, URLNormalizer, showNotification, opened_db, seen_link, next_fetch_at, get_time_diff, get_newshelper_db, get_recent_report, check_recent_seen, sync_report_data, log_browsed_link, check_url, check_report, newshelperWidgetIcon, newshelperWidgetPageIcon, lastActiveTabResult, newshelperPanel, newshelperWidget;
  ...
  ref$ = require('sdk/timers'), setTimeout = ref$.setTimeout, clearTimeout = ref$.clearTimeout;
  ...
  sync_report_data = function(){
    return get_newshelper_db(function(opened_db){
      return get_recent_report(function(report){
        var cachedTime, url;
        cachedTime = Math.max(next_fetch_at, (report != null ? report.updated_at : void 8) != null ? parseInt(report.updated_at) : 0);
        url = "http://newshelper.g0v.tw/index/data?time=" + cachedTime;
        return Request({
          url: url,
          onComplete: function(response){
            var ret, transaction, objectStore, next_fetch_at, i, updateDbTime, ref$;
            ret = response.json;
  ...
  newshelperWidget = ActionButton({
    id: 'newshelper-icon',
    label: "新聞小幫手",
    icon: {
      '64': newshelperWidgetIcon
    },
    panel: newshelperPanel,
    onClick: function(){
      newshelperPanel.show({
        position: newshelperWidget
      });
